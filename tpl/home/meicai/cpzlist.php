...
$D = db('type');
$cid = $DATA['id'];
$fruit = $D->where(array('cid'=>$cid,'off'=>2))->select();
if(!$fruit){
    $cid = $DATA['cid'];
    $fruit = $D->where(array('cid'=>$cid))->select();
}
            ...
            <div class="oc clearfix pull-left f-li ">
                ...
                <div class="f-select pull-left clearfix">
                    <div class="pull-left leftpart">
                        <a href="?<?php echo $DATA['url']?>.html&cid=<?php echo $DATA['cid']?>" class="<?php if($_GET['cid'] != $DATA['id'])echo 'active'?>">
                            <span class="f-all line">全部</span>
                        </a>
                    </div>
                    <div class="pull-left rightpart">
                    <?php foreach ($fruit as $val) {?>
                        <a href="?<?php echo $val['url']?>.html&cid=<?php echo $val['id']?>" class="<?php if($_GET['cid'] == $val['id'])echo 'active'?>" >
                            <span class="line"><?php echo $val['name']?></span>
                        </a>
                    <?php }?>
